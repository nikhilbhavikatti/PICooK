...
"""Fine-tuning script for Stable Diffusion for text2image as well as images2image with support for LoRA.
Adapted to be conditioned on input images instead of input text"""
...
def parse_args():
    ...
    # @janha custom arguments
    parser.add_argument(
        "--imgs2img",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to do imgs2img training",
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default=None,
        help="Path to data directory for imgs2img training",
    )
    ...
    # Sanity checks
    if args.dataset_name is None and args.train_data_dir is None and args.data_dir is None:
...
DATASET_NAME_MAPPING = {
    ...
    "lmuhammadbilal5110/indian_food_images": ("image", "text") # @janha added food images dataset
}
...
def main():
    ...
    if not args.imgs2img:
        print("Loading text2img dataset")
        # Their original code to load huggingface text2img datasets
        if args.dataset_name is not None:
            ...
            # @janha custom code to use classification dataset
            label_mapping = dataset["train"].features["label"].names
            names = [label_mapping[int(e)] for e in dataset["train"]["label"]]
            dataset["train"] = dataset["train"].add_column("text", names)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # @janha Get the text embedding for conditioning
                if not args.imgs2img:
        ...
        if accelerator.is_main_process:
            if epoch % args.validation_epochs == 0:
                # @janha custom picook diffusion pipeline
                pipeline = PicookDiffusionPipeline.from_pretrained(
